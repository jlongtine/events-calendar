...
class Event
  ...
  attr_accessor :start_date, :end_date, :summary, :description, :location, :rrule, :uid
...
EOF
  ...
  #Add by ravin- undone
  def get_events_new(c_id)
    http = Net::HTTP.new(@google_url, 80)
    response, data = http.get("http://#{@google_url}/calendar/feeds/" + c_id+"/private/full?start-min=2009-06-16T00:00:00&start-max=2009-08-24T23:59:59",@headers)
    puts "3.1"
    case response
    when Net::HTTPSuccess, Net::HTTPRedirection
      #redirect_response, redirect_data = http.get(response['location'],@headers)
...
class ICALParser
  ...
  def handle_vevent_dtstart(value)
    ...
    @calendar.events.last.start_date = value
    #puts "start time :#{Time.parse(value)}"
  end

  def handle_vevent_dtend(value)
    #@calendar.events.last.end_date = Date.parse(value)
    @calendar.events.last.end_date = value
    #puts "end time   :#{Time.parse(value)}"
  end
  ...
  def handle_vevent_uid(value)
    @calendar.events.last.uid = value
  end
...
def scan(ical_url, base_url='www.google.com')
  Net::HTTP.start(base_url, 80) do |http|
    ...
    puts "hhhhhhhhhhhfffhhhhhhhhhhhhhhh"
    case response
    when Net::HTTPSuccess, Net::HTTPRedirection
      return data
    else
