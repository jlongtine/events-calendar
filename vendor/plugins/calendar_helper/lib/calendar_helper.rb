...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    if options[:previous_month_text] or options[:next_month_text]
      cal << "<th colspan='2' class='monthName' ><a onclick=load_calendar(#{options[:year]},#{options[:month]-1})><font size='2'>#{options[:previous_month_text]}</font></a></th>"
    ...
    cal << %(<th colspan="#{colspan}" class="#{options[:month_name_class]}">#{options[:year]}/#{Date::MONTHNAMES[options[:month]].to_s[0,3]}</th>)
    #cal << %(<th colspan="2"><a onclick=load_calendar(#{options[:year]},8)><font size='2'>#{options[:next_month_text]}</font></a></th>) if options[:next_month_text]
    cal << "<th class='monthName' colspan='2'><a onclick=load_calendar(#{options[:year]},#{options[:month]+1})><font size='2'>#{options[:next_month_text]}</font></a></th>" if options[:next_month_text]
    ...
    cal << "</tr></thead><tbody><tr>"
     ...
     if options[:detail_view]=="true"
      cal << "<td onClick=week_filter('#{options[:year]}-#{options[:month]}-1') class='#{options[:week_class]}'><a>&gt;&gt;</a></td>"
    end
    ...
    first.upto(last) do |cur|
      ...
      cal << "<td  onmouseover='cellOver(this,#{cell_text})' onClick=displaymessage(#{cell_text},'#{options[:year]}-#{options[:month]}-#{cell_text}') onmouseout='colorChangeOut(this)' #{cell_attrs}>#{cell_text}</td>"

      if options[:detail_view]=="true" and cur!=last
        cal << "</tr><tr><td onClick=week_filter('#{options[:year]}-#{options[:month]}-#{cell_text.to_i+1}') class='#{options[:week_class]}'><a>&gt;&gt;</a></td>" if cur.wday == last_weekday
      else
